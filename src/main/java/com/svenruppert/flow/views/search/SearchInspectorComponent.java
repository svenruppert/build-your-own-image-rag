 ...
 *
 * <p>Visualises the search pipeline as a vertical timeline of explicit steps, each with
 ...
 *   <li>LLM Query Understanding — the language-model analysis phase</li>
 *   <li>Transformed Query — the rich embedding text produced by the LLM</li>
 *   <li>Derived Parameters — structured filters extracted by the LLM</li>
 *   <li>Vector Search — semantic similarity search against the image index</li>
 *   <li>Results — the final matched image count</li>
 * </ol>
 *
 * <h3>Step states</h3>
 * <ul>
...
public class SearchInspectorComponent extends VerticalLayout {
  ...
  private final StepRow stepVector;
  ...
  public SearchInspectorComponent() {
    ...
    stepLlm         = new StepRow("2", "LLM Query Understanding", "Waiting…");
    stepTransformed = new StepRow("3", "Transformed Query",       "Waiting…");
    stepParams      = new StepRow("4", "Derived Parameters",      "Waiting…");
    stepVector      = new StepRow("5", "Vector Search",           "Waiting…");
    stepResults     = new StepRow("6", "Results",                 "Waiting…");

    add(stepQuery, stepLlm, stepTransformed, stepParams, stepVector, stepResults);
    reset();
  }
  ...
  public void reset() {
    ...
    stepVector.update(StepStatus.PENDING,  "Waiting…",                null);
  ...
  /** Marks step 2 (LLM Analysis) as ACTIVE. */
   ...
   * This is idempotent — safe to call on every poll tick once the plan is available.
   */
  ...
  /** Marks step 5 (Vector Search) as ACTIVE. */
  public void setVectorActive() {
    stepVector.update(StepStatus.ACTIVE, "Searching image index…", null);
  }

  /** Marks steps 5 and 6 as COMPLETED once the vector search has returned results. */
  public void setVectorCompleted(int resultCount) {
    stepVector.update(StepStatus.COMPLETED, "Search complete", null);
  ...
  /**
   * Marks steps 5 and 6 as SKIPPED.
   * Called in TRANSFORM_ONLY mode after LLM analysis completes.
   */
  public void skipExecutionSteps() {
    stepVector.update(StepStatus.SKIPPED,  "Skipped — Transform Only mode", null);
   ...
   *                     (values ≥ 3 indicate the LLM completed; failure was in vector search)
   */
  public void setFailed(String error, int stepProgress) {
    String skippedNote = "Not reached";
    if (stepProgress >= 3) {
      // LLM completed; failure occurred during vector search
      stepVector.update(StepStatus.FAILED,
      ...
      stepVector.update(StepStatus.SKIPPED,      skippedNote, null);
      stepResults.update(StepStatus.SKIPPED,     skippedNote, null);
    }
  }
